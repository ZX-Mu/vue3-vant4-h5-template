...
```vue
<i-icon icon="material-symbols:contact-support-outline-rounded" />
```

项目中使用 `unplugin-icons`、`unplugin-vue-components` 插件进行按需自动安装图标集，配置在vite.config.ts中。
...
```shell
npm install -D @iconify-icons/fa6-solid
```
